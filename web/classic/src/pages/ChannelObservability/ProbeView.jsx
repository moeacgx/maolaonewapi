...
const normalizeProbeChannel = (row) => {
  ...
  return {
    id,
    name: String(row.name || row.channel_name || `渠道 #${id}`),
    type,
    typeName: CHANNEL_TYPES[type] || `类型 ${type}`,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const normalized = all.map(normalizeProbeChannel);
