...
const readImageAttachment = (attachment) =>
  new Promise((resolve, reject) => {
    const file = attachment?.fileInstance;
    const isImage =
...
const Playground = () => {
  ...
  async function onMessageSend(content, attachment = []) {
    if (customRequestMode && attachment.length > 0) {
      Toast.warning({
        content: t('图片功能在自定义请求体模式下不可用'),
        duration: 3,
      });
      return;
    }
    ...
    if (attachment.length > 0) {
      try {
        validImageUrls = (
          await Promise.all(attachment.map(readImageAttachment))
        ).filter(Boolean);
      } catch (error) {
    ...
    if (attachment.length > 0 && validImageUrls.length === 0) {
      Toast.error({
        content: t('无法添加图片'),
        duration: 3,
      });
    ...
    const messageContent = buildMessageContent(content, validImageUrls);
