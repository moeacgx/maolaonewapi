...
const parseOriginalStatusCode = (value) =>
  isValidOriginalStatusCode(value) ? value : undefined;

const parseReplaceStatusCode = (value) =>
  isValidReplaceStatusCode(value) ? value : undefined;
  ...
  try {
    ...
    return value
      ...
      .map((rule) => ({
        ...
        mode: rule.mode,
        status_code: parseOriginalStatusCode(rule.status_code),
        replace: rule.replace,
        replace_status_code: parseReplaceStatusCode(rule.replace_status_code),
      }));
  } catch {
    return [];
  }
