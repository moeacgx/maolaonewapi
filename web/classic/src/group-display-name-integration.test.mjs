...
test('模型广场消费 group_names 但筛选和计价仍使用内部 code', () => {
  ...
  assert.match(billingSource, /getGroupDisplayName\(group\.value, groupNames\)/)
  assert.match(
    performanceSource,
...
test('Classic 任务日志分组显示当前名称但仍用内部 code 着色', () => {
  ...
  const hookSource = readSource('hooks/task-logs/useTaskLogsData.js')

  assert.match(columnSource, /record\?\.group_name \|\| group/)
  assert.match(columnSource, /colors\[group\.length % colors\.length\]/)
  assert.doesNotMatch(columnSource, /\{group\}\s*<\/Tag>/)
  assert.match(hookSource, /task\.group_name \|\| task\.group \|\| '-'/)
})
