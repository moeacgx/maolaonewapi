...
*/
import {
  searchChannels as searchManagedChannels,
  testChannel as testManagedChannel,
...
export async function getProbeChannels(): Promise<ProbeChannel[]> {
  ...
  while (hasMore && page <= 101) {
    const response = await searchManagedChannels({
      keyword: '',
      group: '',
      model: '',
      id_sort: true,
    ...
    const total = response.data.total ?? result.length
    page += 1
...
export async function testProbeChannel(channelId: number, model: string) {
  ...
  const response = await testManagedChannel(channelId, { model })
  if (!response.success) {
    throw new Error(response.message || response.data?.error || 'Probe failed')
  }
