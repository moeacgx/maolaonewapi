...
export function StabilityView({
  ...
  const renderRows = (
    ...
    return items.flatMap((item) => {
      ...
      let title = modelName || t('Unknown model')
      if (level === 'group') {
        title = item.group_name || item.group || t('Unlabelled group')
      } else if (level === 'channel') {
        title = item.channel_name || `#${item.channel_id}`
      }

      let subtitle = ''
      if (level === 'group') {
        if (item.group_name && item.group_name !== item.group) {
          subtitle = item.group || ''
        }
      } else if (level === 'channel') {
        subtitle = `#${item.channel_id} · ${item.channel_type_name || t('Unknown type')}`
      } else if (
        item.requested_model &&
        item.upstream_model &&
        item.requested_model !== item.upstream_model
      ) {
        if (modelDimension === 'upstream') {
          subtitle = `${t('Requested model')}: ${item.requested_model}`
        } else {
          subtitle = `${t('Upstream model')}: ${item.upstream_model}`
        }
      }
      const row = (
        <TableRow
