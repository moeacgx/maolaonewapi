...
const EXTENSIONS_QUERY_KEY = getExtensionQueryKey()
    ...
    <div className='mx-auto flex w-full max-w-6xl flex-col gap-4 p-4 md:p-6'>
      <div className='flex flex-col gap-3 md:flex-row md:items-center md:justify-between'>
        <div className='min-w-0'>
          ...
          <p className='text-muted-foreground mt-1 text-sm'>
            {t(
              'Upload a zip module archive or drop modules into the modules directory, then refresh here.'
            )}
          </p>
        </div>
        ...
        <CardContent>
          ...
          {!isLoading && modules.length === 0 && (
            <Empty className='border'>
              ...
              <EmptyDescription>
                {t(
                  'Upload a module zip archive or create a module folder with a manifest.json file.'
                )}
              </EmptyDescription>
            </Empty>
          )}
      ...
      <ConfirmDialog
        ...
        desc={
          <span>
            {t(
              'This will delete the module files and remove its enabled state.'
            )}
            {deleteTarget ? (
              <>
                {' '}
    ...
    <Table>
      ...
      <TableBody>
        {modules.map((module) => {
          ...
          let pageTrigger = <button type='button' />
          if (module.enabled && firstPage) {
            pageTrigger = (
              <Link
