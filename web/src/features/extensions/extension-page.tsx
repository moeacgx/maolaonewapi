...
import { getExtensions, getExtensionPageUrl } from './api'
import { NativeExtensionPage } from './native-page'
...
function syncExtensionFrameTheme(frame: HTMLIFrameElement | null) {
  ...
  try {
    frameRoot = frame.contentDocument?.documentElement ?? null
  } catch {
    // Sandboxed scripted frames have an opaque origin; use postMessage below.
  }
  if (frameRoot) {
...
export function ExtensionModulePage({
  ...
  const invalidProxyPage =
    page?.render?.type !== 'native' && !page?.path?.trim()
  const staticModuleVersion =
    module?.runtime?.type === 'static' ? module.version : ''
  const src = page?.path
    ? getExtensionPageUrl(moduleId, page.path, staticModuleVersion)
    : ''
  ...
  if (!module || !module.enabled || module.error || !page || invalidProxyPage) {
    return (
      <div className='flex min-h-[60vh] items-center justify-center p-6'>
        <Empty className='max-w-md border'>
          <EmptyMedia variant='icon'>
