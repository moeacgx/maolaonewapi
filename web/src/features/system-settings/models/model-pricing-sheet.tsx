...
function parsePriceVariantExpression(
  ...
  const parseLine = (rawLine: string, lineNumber: number) => {
    ...
    if (parts.length === 1 && parts[0].startsWith('sku_out_')) {
      ...
      const qualityPart = skuParts.at(-1)
      if (skuParts.length >= 2 && qualityPart !== undefined) {
        parts = [skuParts.slice(0, -1).join('_'), qualityPart]
      }
    }
            ...
            {expressionPreview.draft ? (
              <div className='rounded-lg border'>
                ...
                <div className='max-h-56 overflow-auto'>
                  {expressionPreview.draft.rules.map((rule) => (
                    <div
                      key={rule.id}
