...
export function InvoiceSettingsVisualEditor({
  ...
  const nextRuleEditorId = React.useRef(0)
  const ruleEditorIds = React.useRef<string[]>([])
  while (ruleEditorIds.current.length < rules.length) {
    ruleEditorIds.current.push(`invoice-rule-${nextRuleEditorId.current++}`)
  }
  ...
  const ruleRows = rules.map((rule, index) => {
    ...
    if (editorId === undefined) {
      throw new Error('Invoice rule editor ID is missing')
    }
    return { editorId, rule }
  })
  ...
  const patchRule = (
    ...
    const nextRows = ruleRows
      .map(({ editorId, rule }, idx) => {
        if (idx !== index) return { editorId, rule }
        ...
        return { editorId, rule: merged }
      })
      .sort((a, b) => a.rule.min - b.rule.min)
    ruleEditorIds.current = nextRows.map(({ editorId }) => editorId)
    onFeeRulesChange(serializeRules(nextRows.map(({ rule }) => rule)))
  }
  ...
  const addRule = () => {
    ...
    const nextRows = [
      ...ruleRows,
      {
        editorId: `invoice-rule-${nextRuleEditorId.current++}`,
        rule: { min: nextMin, type: 'fixed' as const, value: 0 },
      },
    ].sort((a, b) => a.rule.min - b.rule.min)
    ruleEditorIds.current = nextRows.map(({ editorId }) => editorId)
    onFeeRulesChange(serializeRules(nextRows.map(({ rule }) => rule)))
  ...
  const deleteRule = (index: number) => {
    const retainedRows = ruleRows.filter((_, idx) => idx !== index)
    const nextRules =
      retainedRows.length > 0
        ? retainedRows.map(({ rule }) => rule)
        : DEFAULT_RULES
    ruleEditorIds.current =
      retainedRows.length > 0
        ? retainedRows.map(({ editorId }) => editorId)
        : DEFAULT_RULES.map(() => `invoice-rule-${nextRuleEditorId.current++}`)
    onFeeRulesChange(serializeRules(nextRules))
  }
      ...
      <div className='rounded-lg border'>
        ...
        <div className='hidden md:block'>
          <Table>
            ...
            <TableBody>
              {ruleRows.map(({ editorId, rule }, index) => (
                <TableRow key={editorId}>
                  <TableCell>
                    <Input
                      type='number'
                      min={0}
        ...
        <div className='divide-y md:hidden'>
          {ruleRows.map(({ editorId, rule }, index) => (
            <div key={editorId} className='space-y-3 p-4'>
