...
export function SensitiveWordsSection({
  ...
  const getRuleSummary = (rule: SensitiveRuleDraft) => {
    ...
    let scope = t('Request')
    if (rule.scope === SCOPE_RESPONSE) {
      scope = t('Response')
    } else if (rule.scope === SCOPE_BOTH) {
      scope = t('Both')
    }

    let target = `${t('Specified channels')} (${rule.channelIds.length}) · ${t('Specified groups')} (${rule.groupCodes.length})`
    if (rule.targetType === TARGET_ALL) {
      target = t('All channels')
    } else if (rule.targetType === TARGET_CHANNEL_TAGS) {
      target = `${t('Legacy channel groups')} (${rule.channelTags.length})`
    }
          ...
          {rules.length === 0 ? (
            ...
            <div className='space-y-3'>
              {rules.map((rule, index) => {
                const targetsAllChannels = rule.targetType === TARGET_ALL
                const targetsLegacyGroups =
                  rule.targetType === TARGET_CHANNEL_TAGS
                const targetsSelectedRoutes =
                  !targetsAllChannels && !targetsLegacyGroups
                    ...
                    {isRuleExpanded(rule.id) ? (
                      ...
                      >
                        ...
                        <div className='space-y-1.5'>
                          ...
                          <p className='text-muted-foreground text-xs'>
                            {t(
                              'Empty lines and duplicate keywords are ignored.'
                            )}{' '}
                            {t(
