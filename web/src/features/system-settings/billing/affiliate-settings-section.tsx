...
function applicationActionPrompt(
  ...
  if (action === 'approve') return t('Approve this application?')
  if (action === 'reject') return t('Reject this application?')
  return t(
...
function fraudAlertStatusVariant(status: string): BadgeVariant {
  if (status === 'detected') return 'destructive'
...
function AdminTablePagination({
  ...
  const pageItems = pageNumbers.map((pageNumber, index) => {
    ...
    const previousPage = pageNumbers[index - 1] ?? 'start'
    const nextPage = pageNumbers[index + 1] ?? 'end'
    return {
...
export function AffiliateSettingsSection({ defaultValues }: Props) {
  ...
  const handleAppAction = async (
    ...
    const msg = applicationActionPrompt(t, action)
    const reason =
      action === 'reject'
  ...
  const showNoApplications = !appsLoading && applications.length === 0
  const showApplications = !appsLoading && applications.length > 0
  const showNoFraudAlerts = !fraudLoading && fraudAlerts.length === 0
  const showFraudAlerts = !fraudLoading && fraudAlerts.length > 0
                ...
                fraudAlerts.map((alert) => {
                  ...
                  return (
                    <TableRow key={alert.inviter_id || alert.id}>
                      ...
                      <TableCell>
                        <Badge variant={fraudAlertStatusVariant(alert.status)}>
                          {t(alert.status)}
                        </Badge>
                      </TableCell>
                      <TableCell>
