...
export function LogSettingsSection({
  ...
  const form = useForm<LogSettingsFormValues>({
    ...
    defaultValues: {
      ...
      LogRetentionDays: defaultRetentionDays,
    },
  });
  ...
  }, [defaultEnabled, defaultForceRecordIpEnabled, defaultRetentionDays, form]);
  ...
  const onSubmit = async (values: LogSettingsFormValues) => {
    const updates = [
      ...
      },
      {
        ...
        changed: values.LogRetentionDays !== defaultRetentionDays,
      },
          ...
          <FormField
            ...
            render={({ field }) => (
              <SettingsControlGroup className="space-y-2">
                <div>
                  <FormLabel>{t("Database log retention days")}</FormLabel>
                  <FormDescription>
                    {t(
                      "Set to 0 to disable scheduled cleanup. When enabled, old database log entries are deleted hourly in batches.",
                    )}
                  </FormDescription>
                </div>
                <FormControl>
