...
function getStatusAddress(status: unknown, key: string): string {
  const value = asStatusLike(status)
  return normalizeCCSwitchAddress(value[key] ?? value.data?.[key])
}
...
function readStoredStatus(): StatusLike | undefined {
  ...
  try {
    ...
    return raw ? asStatusLike(JSON.parse(raw)) : undefined
  } catch {
    return undefined
  }
}
...
export function resolveCCSwitchAddresses(
  ...
  const homepage =
    ...
    fallbackAddress
  const configuredApiAddress =
    getStatusAddress(status, 'cc_switch_api_address') ||
    getStatusAddress(storedStatus, 'cc_switch_api_address')
  const apiAddress = isValidCCSwitchAddress(configuredApiAddress)
    ? configuredApiAddress || homepage
    : homepage
