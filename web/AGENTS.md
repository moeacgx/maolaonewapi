...
- 后台页面统一使用“页面外壳平铺、业务模块有边界”的布局习惯：每个独立模块必须有明确的边框、背景和一致圆角，标题/说明/操作区与内容区通过间距或分隔线形成稳定层级；表格、表单、报表、状态、错误和空状态不得直接贴在页面背景上。优先复用已有 Card、Panel、Table、Drawer、Dropdown 等组件与主题变量，避免把多个装饰性卡片层层嵌套；页面级平铺不等于取消业务模块边界。
- Default（`web/src`）和 Classic（`web/classic`）均适用此约束。新增后台页或功能时应在设计和实现阶段一次性采用该容器规范，不依赖用户逐页提醒；若沿用特殊的全屏编辑器、图表画布或 Modal，应在模块周围保留可识别的结构边界并说明例外原因。
...
- `src/` 是 Default 模板，`classic/` 是 Classic 模板；两者拥有独立的页面入口、路由、实现和验证链路，源码修改不会自动同步。
- 开始修改前，先通过实际访问路由、构建入口、页面来源或用户截图确认问题对应的模板；版本号或目录名称不能替代实际确认。
- 按需求范围执行：单模板需求只处理对应模板；只有用户明确要求双模板一致，或已核实问题同时存在且需求覆盖两者时，才分别实现并验证两套模板。
- 提交、PR 或开发记录须写明涉及模板（Default、Classic 或两者）、实际差异边界及测试、构建或截图依据；未修改另一模板时，说明其不在范围内或不受影响。
...
- **2026-08-30**：补充 Default/Classic 模板边界确认规范，要求按需求判断是否需要同步修改并在交付记录中说明模板范围与依据。
