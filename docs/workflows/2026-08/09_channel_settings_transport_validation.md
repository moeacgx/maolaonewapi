...
- Default 前端渠道表单镜像后端约束：选择 HTTP/1.1 时自动把 `http2_connection_shards` 重置为 `1` 并禁用输入；schema 同步拒绝 HTTP/1.1 + HTTP/2 分片大于 1，避免提交后才收到后端错误。
- Default 前端依赖锁同步 `dompurify`、`jsdom` 与 `package.json` / `bun.lock`，移除全局 `brace-expansion@2.1.1` override，避免 `minimatch@10` 解析到不兼容的 `brace-expansion` 导致 ESLint 在配置匹配阶段崩溃。
- ESLint 配置保留 `rules-of-hooks`、`exhaustive-deps`、类型、重复导入等稳定检查，显式关闭当前代码库尚未满足的 React Compiler 规则，保证 `npx eslint .` 可以稳定运行并报告现有 warning，而不是在全仓旧代码上产生配置级 error。
