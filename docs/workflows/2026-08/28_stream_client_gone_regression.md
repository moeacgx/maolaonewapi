...
Flush 前的入口上下文取消；另外使用 `httptest.Server` 验证普通、表单和任务三条
HTTP 转发路径在入口取消后都会收到上游 context 取消，并验证已排队的流数据不会在
客户端断开后继续执行。未修改前端流消费逻辑，因此无需前端构建。

修复分支已从 `custom-main` 重新建立，只移植流式修复、回归测试和本工作记录；没有
合入 `frank-pug` 上的其他功能提交。提交到目标分支前必须重新运行仓库 Backend、
Frontend 和 PR Check，确认 CI job 有实际步骤日志并全部通过。

错误归因边界：响应体关闭错误只有在确认由客户端取消触发的 cleanup 后才归类为
`client_gone`；客户端仍在线时产生的同名上游读取错误继续保留为真实扫描器错误。
