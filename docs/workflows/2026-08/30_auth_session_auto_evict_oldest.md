...
- `USER_SESSION_ISSUANCE_LIMIT` 与 `USER_SESSION_ISSUANCE_WINDOW_SECONDS`
  仍统计窗口内全部创建记录（含 revoked 和旧鉴权版本），达到窗口上限时
  返回 `429 AUTH_SESSION_ISSUANCE_LIMIT`。自动撤销不能绕过签发窗口限制。
...
成功，但数据库提交后的未过期 active 行数始终不超过配置上限，且总创建量仍受
签发窗口限制。
...
- 并发登录最终 active 数不超过上限；签发窗口仍限制短时间内的总创建量。
