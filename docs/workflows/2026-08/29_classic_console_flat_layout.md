...
统一 Classic 控制台七个独立页面的页面外壳，使固定顶栏下的主内容具有一致的顶部占位、最大宽度、居中和水平内边距。重点修复营销福利页缺少最大宽度，以及个人设置页重复居中的问题。
...
- `.classic-console-page-container` 使用 `width: 100%`、`max-width: 1440px`、`margin: 0 auto` 和 8px 水平内边距。
- 页面业务内容放入容器；弹窗保持为页面外壳的兄弟节点，避免改变 Semi UI 的弹层行为。
- 内部业务卡片与 Tabs 不因外壳统一而拆除，既有数据请求、权限和交互契约保持不变。

## 兼容性与风险

这是 Classic 前端的布局层变更，不改变 API、请求参数、权限、计费或后端数据。审计页原有 1600px 外层宽度统一收敛到 1440px；其内部 Card、表格和 Tabs 保持不变，超宽内容仍由组件自身滚动策略处理。
