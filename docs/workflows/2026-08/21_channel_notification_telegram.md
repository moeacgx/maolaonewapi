...
- 事件类型展示按稳定的事件值映射到前端翻译键；后端继续返回英文规范标签，避免后端标签文案变化导致多语言回退。
- 消息模板下方的变量按钮支持点击插入：有选中文本时替换选区，否则插入到当前光标位置，并在插入后恢复编辑焦点。
- 变量说明同时展示用途和示例值；扩展事件中未内置说明的变量使用通用事件负载说明。
- 该改动不改变通知任务请求字段、模板变量语法或 Telegram 投递行为。部署时必须重新构建前端静态资源，并在浏览器或反向代理仍缓存旧资源时执行缓存失效/硬刷新。

## 历史模板兼容修复（2026-08-28）

早期 Default 与 Classic 通知中心在切换事件类型时只更新 `event_type`，没有替换仍为发票默认值的模板。用户从 `invoice_pending` 切换到 `channel_disabled` 后，发票默认模板中的 `{{total_amount}}` 会按渠道事件负载校验失败；历史上已保存的同类任务则会在异步渲染阶段失败。

修复保持事件变量白名单和未知变量拒绝不变：前端仅在当前模板为空或等于原事件默认模板时换用目标事件默认模板；后端将核心渠道事件的空模板或完整历史发票默认模板按目标事件归一化，并在任务加载、保存和发送时生效；发票事件仍保留 `total_amount`。任意自定义的 `{{total_amount}}` 仍会被渠道事件校验拒绝。Telegram 请求字段和渠道事件负载契约不变。
...
- Default 前端生产构建通过。
- 本次前端改动通过通知中心列表测试、`tsgo -b`、目标文件 `oxlint`、i18n 同步报告和生产构建；本机未安装 `bun`，同步与构建使用已安装的 Node/本地工具等价执行。
- 历史模板兼容回归覆盖核心事件默认归一化、发票 `total_amount` 渲染、未知变量拒绝和 Telegram HTML 请求字段；Default/Classic 切换测试确认未修改的自定义模板保留、默认模板切换到目标事件。
