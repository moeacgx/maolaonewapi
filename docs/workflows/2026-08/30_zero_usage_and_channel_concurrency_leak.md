...
此前渠道并发计数是每个 Go 进程的内存状态。自动渠道测试直接调用
...
该问题的槽位泄漏已在 `282` 修复；三容器合计计数则由后续 Redis 租约修复完成，具体见
[三容器渠道并发统一计数](30_channel_concurrency_multi_instance.md)。
...
- `Distribute` 统一入口原有的请求级释放逻辑保持不变；Redis 租约在请求结束时精确释放，异常退出时由 TTL 回收。
...
- Redis 开启时有限并发是三个连接同一 Redis 逻辑数据库的容器合计值；Redis 故障时有限渠道 fail-closed，不回退为每容器独立计数。Redis 未启用时只保留单实例本地测试回退。
- 生产发布仍需逐实例进行；滚动更新期间各容器使用同一 Redis 租约集合，健康检查与租约回收必须一并观察。
