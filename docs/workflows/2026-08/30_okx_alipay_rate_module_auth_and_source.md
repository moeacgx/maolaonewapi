...
OKPay 自动汇率缓存最初只跟随来源和模块配置变化，未把模块启用状态纳入判定。
模块被禁用后，新订单仍可能在缓存 TTL 内继续使用旧的模块汇率。
...
- OKPay 模块源缓存键包含模块启用状态，模块禁用后不会继续命中旧的模块汇率。
...
- Default 模型广场卡片和详情页同步展示同一综合折扣，且只在综合因子低于原价时
  展示折扣徽标。
...
模块禁用会立即让 OKPay 模块源缓存失效。后续新订单会回退到手动兜底汇率，
不会继续使用禁用前缓存的 OKX 报价。

模型广场卡片和详情页只展示真实折扣徽标：综合因子低于原价时展示折数；
综合因子等于原价或高于原价时不显示折扣徽标。
...
- OKPay 控制器测试覆盖模块汇率成功缓存后禁用模块，下一次取值会立即回退到
  手动兜底汇率。
...
- Default 定价契约覆盖综合折扣仍由 `price / usd_exchange_rate` 与分组倍率共同
  计算，并确认卡片与详情页继续接入 `getBillingCompositeFactor`，详情页不会为
  原价或加价因子展示折扣徽标。
