...
- `USER_SESSION_ACTIVE_LIMIT`（默认 `50`）：单用户未过期且状态为 active 的 Session 容量。达到容量时，新登录会在同一准入事务中按 `last_active_at`、`created_at` 和 `sid` 顺序淘汰最旧会话，再签发新会话。
...
活跃会话容量只控制同时保留的 Session 数量；达到容量时不会阻止新登录，最旧的活跃会话会被标记为 `revoked`，原因是 `login_session_evicted`。签发窗口限制仍会拒绝过于密集的新登录。
...
活跃数量会计入状态仍为 active 但 `user_auth_version` 已过期的异常残留行，而设备列表只展示当前鉴权版本；新登录淘汰旧会话时也会覆盖这类残留。密码重置仍会撤销全部会话，但不会清空签发窗口计数。
