...
- [zzapi WebSocket 徽标发布记录](../workflows/2026-09/02_zzapi_ws_badge_release.md)：以 zzapi 当前 `.296` Responses WebSocket 运行版本为基线，叠加使用日志徽标与操练场修复发布 `.297`。
- [使用日志 WebSocket 徽标](../workflows/2026-09/02_usage_log_ws_badge.md)：Default 与 Classic 使用日志依据当前 `transport` 或历史 `ws` 字段显示明确的 `WS` 徽标，并保持普通 HTTP/SSE 流式请求不误标。
