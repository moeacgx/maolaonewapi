...
- [Codex Responses Lite 并行工具调用兼容](../workflows/2026-08/25_codex_responses_lite_parallel_tool_calls.md)：Codex 渠道在 Responses Lite 请求中明确将 `parallel_tool_calls` 固定为 `false`，覆盖转换与请求体透传路径，避免上游内部模式拒绝默认并行工具调用。
