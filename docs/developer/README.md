...
- [通用动态路由](../workflows/2026-08/23_dynamic_routing.md)：新增独立系统设置页和渠道覆盖规则，支持普通模型重定向，以及 Responses 图片工具到目标 Responses 或 `/v1/images/generations` 的桥接与目标图片模型计费。
- [渠道列表分组显示名称修复](../workflows/2026-08/21_channel_list_group_display_name.md)：渠道列表响应补齐 `group_details`，Classic 分组列显示当前分组名称而非稳定 code。
