...
- 每个归档目标有稳定 ID；切换活动目标只影响新任务。保留期内存在队列任务或对象时
  ...
  后再切换，避免精确清理误删或遗留旧对象。
- 完整请求归档、启用 Guard 门禁或保存 Guard/对象存储令牌前仍必须显式配置稳定的
  `CRYPTO_SECRET`。密钥丢失或更换后，已有密文无法恢复；轮换前必须先完成数据
  重加密。屏蔽词和上游策略事件即使没有密钥，也会保存审核正文并在 Root 详情页
  展示，不再退化为只有哈希的元数据记录。
...
`CRYPTO_SECRET` 时请求归档 Worker 不领取任务，保持 `queued` 或 `retry` 等待配置恢复，避免把
