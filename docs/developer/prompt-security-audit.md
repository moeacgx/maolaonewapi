...
安全优先语义在分配前执行，避免为了确定最终随机渠道而先占用并发。分组规则按渠道
实际绑定的业务分组匹配，再与当前路由候选集和模型能力求交；不会把 `Channel.Tag`
标签或关键词组当成规则目标。数据库预检同时要求渠道真实存在且
处于启用状态，孤儿能力记录和禁用渠道均不能触发规则。候选集或模型无法可靠解析时采用
fail-safe 预检。响应、SSE 和 Realtime 上游帧按当前尝试实际选中的渠道及其业务分组
精确选择规则；跨渠道重试必须同步刷新选中渠道快照。
