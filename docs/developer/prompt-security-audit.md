...
`cyber_policy_window_hours`。前者只统计当前时间向前回溯配置窗口内的精确官方风控事件，
查询条件与自动封禁完全一致；后者说明本次累计使用的窗口小时数。统计不受列表当前的
来源、日期或关键词筛选影响，用于让 Root 直接判断该用户距离自动封禁阈值还有多少次。

事件同时固化本次路由的渠道和分组元数据。`channel_id`、`channel_name` 记录事件发生时
实际选中的渠道，`group_id`、`group_name` 记录本次请求实际使用的路由分组；
