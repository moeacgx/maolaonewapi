...
判断、渠道分配、计费和协议转换之前把客户端原始 HTTP 正文加密入队。
...
Safe 且不保存通过事件的任务完成后会清空该临时副本。关闭归档后不再接收新任务，但仍保留一个 drain Worker 处理已入队任务。缺少稳定
...
数据库迁移只做向前兼容的表、列和索引新增，默认关闭时不改变现有转发行为。回滚时
先切换为 `off` 并停止 Worker，历史表和事件保留，任何物理删除都必须另行备份和确认。
