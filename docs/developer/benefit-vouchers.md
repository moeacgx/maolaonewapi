...
金额以整数分保存，`fixed_amount_cents * total_count` 必须等于总预算。随机模式先
验证 `count * min <= total <= count * max`，再预拆出确定份额，领取时从可用份额中
随机抽取。`total_quota` 是实际计费额度，份额总和必须严格相等。
...
- Classic 后台使用与优惠码一致的右侧抽屉创建/编辑活动；活动列表保留标题和标签分隔线、
  表格列头、行分隔与独立操作列。表单字段必须展示单位、约束和示例，金额使用整数分、
  额度使用计费 quota、有效期使用秒，活动起止时间固定按 `Asia/Shanghai` 解释；
- 报表、券列表、单券流水和单券作废；
