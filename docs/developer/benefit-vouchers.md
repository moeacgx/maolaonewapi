...
流水接口和数据表继续保留用于审计与对账，但 Default/Classic 用户福利页、Default 管理员券列表
以及 Classic `/console/redemption` 的管理员券列表不再提供“查看流水”入口。
...
- 报表、券列表和单券作废；流水接口继续保留为审计能力，不在上述页面提供查看入口；
...
稳定分隔。券卡片仍作为可重复操作项保留独立边界；报表和券列表在面板内部继续使用分组边界，
避免把表格或空状态直接贴在页面背景上。管理员券列表的操作列固定在表格右侧，随容器宽度铺开。
