...
- AtlasCloud 图像与视频渠道：`AtlasCloud`（类型 61）作为独立渠道承接图像生成/编辑和视频任务，短模型名到 `provider/model/task` 上游模型名由渠道模型重定向配置管理；任务日志中的媒体预览走本机 content 代理，避免浏览器直连上游媒体 URL。
- 腾讯混元渠道：类型 23 会按密钥格式自动分流，`AppId|SecretId|SecretKey` 继续走原生 TC3 签名；单段 TokenHub API Key 走腾讯 TokenHub 的 OpenAI 兼容端点，留空或默认 base URL 会改写为 `https://tokenhub.tencentmaas.com`。
- Gateway 路由兼容：官方的错误归一、Retry-After、容量错误回落、渠道切换和重试优化可以继续同步；但令牌绑定多分组、`selected-channel-group` 传播、亲和多 key 绑定和计费/重试会话语义属于本 fork 的稳定契约，不能被单分组官方实现覆盖。
