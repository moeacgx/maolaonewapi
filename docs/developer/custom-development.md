...
- 客户端错误消息替换：系统设置支持按包含、精确或 RE2 正则配置多条有序规则，首条命中后整体替换最终 OpenAI、Claude、SSE 或 Realtime 客户端文案。规则只在最终响应边界应用，上游原文继续用于自动重试、渠道禁用、安全审计、违规计费、日志和模型广场统计；管理员配置的 403 重试范围优先于渠道亲和失败跳过。完整契约见 [客户端错误替换、403 重试与邀请权限修复](../workflows/2026-08/06_client_error_retry_and_invitation_permission.md)。
