...
- 客户端错误响应替换：系统设置支持用可选原状态码与包含、精确或 RE2 正则文案共同匹配多条有序规则，首条命中后可覆盖尚未提交的最终客户端 HTTP 状态码，并替换 OpenAI、Claude、SSE 或 Realtime 提示词。规则只在最终响应边界应用，上游原文和内部状态码继续用于自动重试、渠道禁用、安全审计、违规计费、日志和模型广场统计；已经提交的 SSE/WebSocket 只能替换流内提示词。管理员配置的 403 重试范围优先于渠道亲和失败跳过。完整契约见 [客户端错误替换、403 重试与邀请权限修复](../workflows/2026-08/06_client_error_retry_and_invitation_permission.md)。
