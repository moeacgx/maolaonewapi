...
- 用户资料与计费隔离：设置、管理访问令牌、身份绑定及管理资料使用显式列或字段白名单更新；通用 `User.Update` 只允许用户名、显示名和显式密码。设置 JSON 通过字节精确旧值 CAS 合并并发修改，管理员资料以目标旧角色为 CAS 守卫，角色和分组更新也检查旧值。Redis 用户快照使用每用户 generation 栅栏并保留带有效用户标记的实时额度；缓存缺失时通过跨实例回退锁把待处理额度立即落库。额度接口区分可中止预扣与已承诺结算/退款，后者落库失败会进入受保护队列并按已接受处理，Redis 关闭时也禁止读取本实例尚未落库的旧额度。令牌已经扣除但钱包或订阅预留失败时，非批量模式使用唯一操作键持久补偿，批量模式在按令牌刷盘锁内原地抵消；Realtime 结算意图不可被错误清理阶段改成全额退款。普通资金和令牌结算没有持久幂等终态，失败后不得由后台自动重放。订阅额度不足使用业务哨兵返回 403，数据库或事务故障保持错误链并返回 503。四个高频自助入口按认证用户 ID 原子限流，`PUT /api/user/self` 另有 64 KiB 请求体和 16 KiB 侧边栏 JSON 限制。完整边界见[用户自助资料更新覆盖计费字段修复](../workflows/2026-08/05_user_self_accounting_lost_update.md)和[计费预留补偿一致性](../workflows/2026-08/07_billing_reservation_consistency.md)。
...
- 客户端错误响应替换：系统设置支持用可选原状态码与包含、精确或 RE2 正则文案共同匹配多条有序规则，首条命中后可覆盖尚未提交的最终客户端 HTTP 状态码，并替换 OpenAI、Claude、SSE 或 Realtime 提示词。数据库“使用日志”同步展示替换后的公开文案但保留内部状态码前缀；上游原文和内部状态码继续用于自动重试、渠道禁用、安全审计、违规计费、运行日志和模型广场统计。已经提交的 SSE/WebSocket 只能替换流内提示词。管理员配置的 403 重试范围优先于渠道亲和失败跳过。完整契约见 [客户端错误替换、403 重试与邀请权限修复](../workflows/2026-08/06_client_error_retry_and_invitation_permission.md)。
