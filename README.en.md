...
| `USER_SESSION_ISSUANCE_LIMIT` | Maximum Sessions created per user within the issuance window, including revoked Sessions; `0` disables issuance limiting | `0` |
| `USER_SESSION_ISSUANCE_WINDOW_SECONDS` | Per-user Session issuance window; even with `USER_SESSION_ISSUANCE_LIMIT=0`, it protects expired/revoked Session cleanup; values above revoked retention are always clamped | `86400` |
