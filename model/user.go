...
type User struct {
	...
	CyberPolicyCountResetEventId int64                      `json:"-" gorm:"type:bigint;not null;default:0;column:cyber_policy_count_reset_event_id"`
...
func (user *User) BeforeCreate(tx *gorm.DB) error {
	...
	group, err := GetGroupByCodeOrAliasWithDB(tx, user.Group)
	if err != nil {
		return err
	}
	user.Group = group.Code
...
func GetUserIdByAffCode(affCode string) (int, error) {
	...
	var user User
	if err := DB.Select("id", "status").First(&user, "aff_code = ?", affCode).Error; err != nil {
		return 0, err
	}
	if user.Status != common.UserStatusEnabled {
		return 0, errors.New("该邀请码已失效")
	}
	if IsAffiliateUserInviteCodeBlocked(user.Id) || !AffiliateUserCanInvite(user.Id, setting.GetAffiliateSetting()) {
		return 0, errors.New("该邀请码已失效")
	}
	return user.Id, nil
}
...
func registerAffiliateInviteTx(tx *gorm.DB, inviterId int, inviteeId int) error {
	if inviterId <= 0 {
		return nil
	}
	var inviter User
	...
	blocked, err := queryAffiliateUserInviteCodeBlockedWithDB(tx, inviterId)
	if err != nil {
		return err
	}
	canInvite, err := queryAffiliateUserCanInviteForUpdateWithDB(tx, inviterId, setting.GetAffiliateSetting())
	if err != nil {
		return err
	}
	if inviter.Status != common.UserStatusEnabled || blocked || !canInvite {
		return errors.New("该邀请码已失效")
	}
	if operation_setting.IsPaymentComplianceConfirmed() && common.QuotaForInvitee > 0 {
		if err := creditTopUpQuota(tx, inviteeId, int64(common.QuotaForInvitee), nil); err != nil {
			return err
		}
	}
	result := tx.Model(&User{}).Where("id = ?", inviterId).
...
func (user *User) Insert(inviterId int) error {
	if err := DB.Transaction(func(tx *gorm.DB) error {
		return withNormalizedEmailLock(tx, user.Email, func(tx *gorm.DB) error {
			...
			return registerAffiliateInviteTx(tx, inviterId, user.Id)
		})
	}); err != nil {
		return err
	}
...
func (user *User) UpdateWithTx(tx *gorm.DB, updatePassword bool) error {
	...
	if newUser.Group == "" {
		// Partial profile updates do not own group identity. Ignore a detached ID
		// unless the authoritative code is present and can be resolved with it.
		newUser.GroupId = 0
	}
	if newUser.Group != "" && tx.Migrator().HasTable(&Group{}) {
		group, resolveErr := GetGroupByCodeOrAliasWithDB(tx, newUser.Group)
		if resolveErr != nil {
			return resolveErr
		}
		newUser.Group = group.Code
	...
	authChanged := (updatePassword && current.Password != newUser.Password) ||
		...
		(newUser.Group != "" && (current.Group != newUser.Group || current.GroupId != newUser.GroupId))
	if authChanged {
...
func (user *User) EditWithTx(tx *gorm.DB, updatePassword bool) error {
	...
	updates := map[string]interface{}{
		"username": newUser.Username, "display_name": newUser.DisplayName,
		"group": newUser.Group, "remark": newUser.Remark,
	}
	...
	authChanged := (updatePassword && current.Password != newUser.Password) || current.Group != newUser.Group || current.GroupId != newUser.GroupId
	if authChanged {
		newUser.AuthVersion, err = IncrementUserAuthVersionWithTx(tx, user.Id)
		if err != nil {
			return err
...
func IncreaseUserQuota[T walletQuotaValue](id int, quota T, db bool) (err error) {
	quota64 := int64(quota)
	if quota64 < 0 {
		return errors.New("quota 不能为负数！")
	}
	gopool.Go(func() {
		err := cacheIncrUserQuota(id, quota64)
		if err != nil {
			common.SysLog("failed to increase user quota: " + err.Error())
		}
	})
	if !db && common.BatchUpdateEnabled {
		addNewRecord(BatchUpdateTypeUserQuota, id, quota64)
		return nil
	}
	return increaseUserQuota(id, quota64)
}
...
// SetUserQuota replaces a wallet balance and invalidates any cached snapshot.
// Admin overrides must not leave a stale 32-bit-era Redis value visible until
// the normal cache TTL expires.
func SetUserQuota(id int, quota int64) error {
	if id <= 0 || quota < 0 {
...
func IncreaseUserQuotaWithTx[T walletQuotaValue](tx *gorm.DB, id int, quota T) (int64, int64, error) {
	quota64 := int64(quota)
	if tx == nil || id <= 0 || quota64 <= 0 {
		return 0, 0, errors.New("invalid transactional user quota increase")
	}
	result := tx.Model(&User{}).Where("id = ?", id).Updates(map[string]any{
		"quota":         gorm.Expr("quota + ?", quota64),
		"quota_version": gorm.Expr("quota_version + 1"),
	})
	if result.Error != nil {
...
func RepairUserQuotaCache(id int, quotaVersion int64, authoritativeQuota int64, amount int64) error {
	return repairUserQuotaCache(id, quotaVersion, authoritativeQuota, amount)
}

func DecreaseUserQuota[T walletQuotaValue](id int, quota T, db bool) (err error) {
	quota64 := int64(quota)
	if quota64 < 0 {
		return errors.New("quota 不能为负数！")
	}
	gopool.Go(func() {
		err := cacheDecrUserQuota(id, quota64)
		if err != nil {
	...
	return decreaseUserQuota(id, quota64)
}
...
func DeltaUpdateUserQuota[T walletQuotaValue](id int, delta T) (err error) {
	delta64 := int64(delta)
	if delta64 == 0 {
		return nil
	}
	if delta64 > 0 {
		return IncreaseUserQuota(id, delta64, false)
	} else {
		return DecreaseUserQuota(id, -delta64, false)
	}
}
...
func UpdateUserUsedQuotaAndRequestCount[T walletQuotaValue](id int, quota T) {
	quota64 := int64(quota)
	if common.BatchUpdateEnabled {
		addNewRecord(BatchUpdateTypeUsedQuota, id, quota64)
		addNewRecord(BatchUpdateTypeRequestCount, id, 1)
		return
	}
	updateUserUsedQuotaAndRequestCount(id, quota64, 1)
}

// UpdateUserUsedQuota adjusts accumulated usage without changing request count.
func UpdateUserUsedQuota[T walletQuotaValue](id int, quota T) {
	quota64 := int64(quota)
	if common.BatchUpdateEnabled {
		addNewRecord(BatchUpdateTypeUsedQuota, id, quota64)
		return
	}
	if err := DB.Model(&User{}).Where("id = ?", id).Update("used_quota", gorm.Expr("used_quota + ?", quota64)).Error; err != nil {
...
func UpdateUserUsedQuotaWithTx[T walletQuotaValue](tx *gorm.DB, id int, quota T) error {
	quota64 := int64(quota)
	if tx == nil || id <= 0 {
		return errors.New("invalid user usage transaction")
	}
	result := tx.Model(&User{}).Where("id = ?", id).
		Update("used_quota", gorm.Expr("used_quota + ?", quota64))
	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected != 1 {
