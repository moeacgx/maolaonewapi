...
// 保护契约：数据库驱动错误消息可能内联数据值，非调试模式下日志只保留错误码。
...
// 调用点归因到业务代码；DEBUG=true 时恢复参数值与错误原文。
...
func TestGormLoggerSlowThresholdConfiguration(t *testing.T) {
	previousDebug := common.DebugEnabled
	common.DebugEnabled = false
	t.Cleanup(func() { common.DebugEnabled = previousDebug })

	traceSlowQuery := func(threshold string, elapsed time.Duration) string {
		t.Helper()
		t.Setenv("SQL_SLOW_THRESHOLD_MS", threshold)
		var buf bytes.Buffer
		newGormLogger(&buf).Trace(
			context.Background(),
	...
	assert.Empty(t, traceSlowQuery("0", time.Second))
	assert.Empty(t, traceSlowQuery("500", 100*time.Millisecond))
	assert.Contains(t, traceSlowQuery("500", time.Second), "SLOW SQL >= 500ms")
	assert.Contains(t, traceSlowQuery("-1", time.Second), "SLOW SQL >= 200ms")
	assert.Contains(t, traceSlowQuery("3600001", time.Second), "SLOW SQL >= 200ms")
}
