...
// Gemini 没有可无损表达的 Responses custom/freeform 工具。转换前同时移除
// custom tool、对应调用及其输出，避免留下孤立 tool result。
...
func filterGeminiResponsesInput(raw []byte) ([]byte, error) {
	...
	for _, item := range items {
		...
		case geminiResponsesInputTypeFunctionCallOutput:
			if _, skipped := skippedCustomCallIDs[strings.TrimSpace(common.Interface2String(item["call_id"]))]; skipped {
				continue
			}
		}
