...
func TestManagerScanRejectsMismatchedAndDuplicateManifestIDs(t *testing.T) {
	rootDir := t.TempDir()
	manifest := Manifest{ID: "shared", Name: "Shared", Version: "1.0.0", Runtime: Runtime{Type: RuntimeTypeHTTP, BaseURL: "https://example.com"}}
	writeManifest(t, rootDir, "first", manifest)
	writeManifest(t, rootDir, "second", manifest)
	manager := NewManager(rootDir)
	require.NoError(t, manager.Scan())
	_, exists := manager.Get("shared")
	require.False(t, exists)
	modules := manager.List(common.RoleRootUser, true)
	require.Len(t, modules, 2)
	for _, module := range modules {
		require.Equal(t, "module manifest is invalid", module.Error)
		require.NotContains(t, module.Error, rootDir)
	}
}

func TestArchiveMetadataRejectsExpandedAndEntryLimits(t *testing.T) {
	tooLarge := &zip.File{FileHeader: zip.FileHeader{UncompressedSize64: uint64(maxInstallArchiveExpandedBytes) + 1}}
	require.ErrorContains(t, validateArchiveMetadata([]*zip.File{tooLarge}), "extracted content")
	tooMany := make([]*zip.File, maxInstallArchiveEntries+1)
	for index := range tooMany {
		tooMany[index] = &zip.File{}
	}
	require.ErrorContains(t, validateArchiveMetadata(tooMany), "entries")

	manager := NewManager(t.TempDir())
	_, err := manager.InstallArchive(bytes.NewReader(nil), MaxInstallArchiveBytes+1)
	require.ErrorContains(t, err, "exceeds")
}

func TestManagerInstallArchiveRejectsDuplicateAndSymlinkEntries(t *testing.T) {
	manifest := Manifest{ID: "unsafe", Name: "Unsafe", Version: "1.0.0", Runtime: Runtime{Type: RuntimeTypeHTTP, BaseURL: "https://example.com"}}
	manifestData, err := common.Marshal(manifest)
	require.NoError(t, err)

	build := func(addUnsafe func(*zip.Writer)) []byte {
		var buffer bytes.Buffer
		writer := zip.NewWriter(&buffer)
		file, createErr := writer.Create("manifest.json")
		require.NoError(t, createErr)
		_, createErr = file.Write(manifestData)
		require.NoError(t, createErr)
		addUnsafe(writer)
		require.NoError(t, writer.Close())
		return buffer.Bytes()
	}
	duplicate := build(func(writer *zip.Writer) {
		file, createErr := writer.Create("manifest.json")
		require.NoError(t, createErr)
		_, createErr = file.Write(manifestData)
		require.NoError(t, createErr)
	...
	manager := NewManager(t.TempDir())
	_, err = manager.InstallArchive(bytes.NewReader(duplicate), int64(len(duplicate)))
	require.ErrorContains(t, err, "duplicate")

	symlink := build(func(writer *zip.Writer) {
		header := &zip.FileHeader{Name: "public/link"}
		header.SetMode(os.ModeSymlink | 0o777)
		file, createErr := writer.CreateHeader(header)
		require.NoError(t, createErr)
		_, createErr = file.Write([]byte("../../outside"))
		require.NoError(t, createErr)
	})
	_, err = manager.InstallArchive(bytes.NewReader(symlink), int64(len(symlink)))
	require.ErrorContains(t, err, "unsupported file")
}

func TestManagerSetEnabledRollsBackMemoryWhenStatePersistenceFails(t *testing.T) {
	rootDir := t.TempDir()
	writeManifest(t, rootDir, "rollback", Manifest{ID: "rollback", Name: "Rollback", Version: "1.0.0", Runtime: Runtime{Type: RuntimeTypeHTTP, BaseURL: "https://example.com"}})
	manager := NewManager(rootDir)
	require.NoError(t, manager.Scan())
	require.NoError(t, os.Mkdir(filepath.Join(rootDir, "state.json"), 0o755))
	_, err := manager.SetEnabled("rollback", true)
	require.Error(t, err)
	module, exists := manager.Get("rollback")
	require.True(t, exists)
	require.False(t, module.Enabled)
	require.False(t, manager.state.Modules["rollback"].Enabled)
}

func TestManagerSerializesConcurrentScanAndStateChanges(t *testing.T) {
	rootDir := t.TempDir()
	writeManifest(t, rootDir, "serialized", Manifest{ID: "serialized", Name: "Serialized", Version: "1.0.0", Runtime: Runtime{Type: RuntimeTypeHTTP, BaseURL: "https://example.com"}})
	manager := NewManager(rootDir)
	require.NoError(t, manager.Scan())
	...
	for index := range 16 {
		...
		go func(enabled bool) {
			defer wait.Done()
			_, _ = manager.SetEnabled("serialized", enabled)
		}(index%2 == 0)
		go func() {
			defer wait.Done()
			_ = manager.Scan()
	...
	wait.Wait()
	module, exists := manager.Get("serialized")
	require.True(t, exists)
	reloaded := NewManager(rootDir)
	require.NoError(t, reloaded.Scan())
	reloadedModule, exists := reloaded.Get("serialized")
	require.True(t, exists)
	require.Equal(t, module.Enabled, reloadedModule.Enabled)
}

func TestInstallBuiltinModulesIncludesChannelQualityNativeAssets(t *testing.T) {
	setTestHostVersion(t, "v1.0.0-rc.99.0.0.0")
	rootDir := t.TempDir()
	require.NoError(t, installBuiltinModules(rootDir))
	manager := NewManager(rootDir)
	require.NoError(t, manager.Scan())
	module, exists := manager.Get("channel-quality")
	require.True(t, exists)
...
func setTestHostVersion(t *testing.T, version string) {
	t.Helper()
...
func TestManifestValidatesStrictHostBounds(t *testing.T) {
	base := Manifest{ID: "hosted", Name: "Hosted", Version: "1.0.0", Runtime: Runtime{BaseURL: "https://example.com"}}
	for _, value := range []string{"v1.2", "v1.2.3.4", "v1.2.03", "v1.2.3-rc..1", "1.2.3-01"} {
		manifest := base
		manifest.Host.Min = value
		require.ErrorContains(t, manifest.Validate(), "host.min is invalid", value)
	}
	for _, value := range []string{"v1.2.3", "1.2.3-rc.10.1.10.204"} {
		manifest := base
		manifest.Host.Min = value
		require.NoError(t, manifest.Validate(), value)
	}
	manifest := base
	manifest.Host.Min = "v2.0.0"
	manifest.Host.Max = "v1.0.0"
	require.ErrorContains(t, manifest.Validate(), "host.min must not exceed host.max")
}

func TestManagerHostCompatibilityFailsClosedAcrossOperations(t *testing.T) {
	setTestHostVersion(t, "v1.0.0-rc.10.1.10.204")
	rootDir := t.TempDir()
	writeManifest(t, rootDir, "incompatible", Manifest{
		ID: "incompatible", Name: "Incompatible", Version: "1.0.0",
		Host: HostCompat{Min: "v2.0.0"}, Runtime: Runtime{BaseURL: "https://example.com"},
	})
	manager := NewManager(rootDir)
	require.NoError(t, manager.Scan())
	module, exists := manager.Get("incompatible")
	require.True(t, exists)
	require.Equal(t, "module is incompatible with current host version", module.Error)
	require.Empty(t, module.Path)
	require.Empty(t, manager.List(common.RoleCommonUser, false))
	listed := manager.List(common.RoleRootUser, true)
	require.Len(t, listed, 1)
	require.Equal(t, "module is incompatible with current host version", listed[0].Error)
	_, err := manager.SetEnabled("incompatible", true)
	require.ErrorContains(t, err, "incompatible")

	archive := buildModuleArchiveWithManifest(t, Manifest{
		ID: "archive-incompatible", Name: "Archive Incompatible", Version: "1.0.0",
		Host: HostCompat{Max: "v0.9.0"}, Runtime: Runtime{BaseURL: "https://example.com"},
	})
	_, err = manager.InstallArchive(bytes.NewReader(archive), int64(len(archive)))
	require.ErrorContains(t, err, "incompatible")
	require.False(t, regularFileExists(filepath.Join(rootDir, "archive-incompatible")))
}

func TestManagerHostCompatibilityAllowsCompatibleModule(t *testing.T) {
	setTestHostVersion(t, "v1.0.0-rc.10.1.10.205")
	rootDir := t.TempDir()
	writeManifest(t, rootDir, "compatible", Manifest{
		ID: "compatible", Name: "Compatible", Version: "1.0.0",
		Host:    HostCompat{Min: "v1.0.0-rc.10.1.10.204", Max: "v1.0.0"},
		Runtime: Runtime{BaseURL: "https://example.com"},
	})
	manager := NewManager(rootDir)
	require.NoError(t, manager.Scan())
	module, err := manager.SetEnabled("compatible", true)
	require.NoError(t, err)
	require.True(t, module.Enabled)
	require.Len(t, manager.List(common.RoleRootUser, false), 1)
}

func buildModuleArchiveWithManifest(t *testing.T, manifest Manifest) []byte {
	t.Helper()
	var buffer bytes.Buffer
	writer := zip.NewWriter(&buffer)
	data, err := common.Marshal(manifest)
